...
// Custom Hook that takes in a username parameter and returns a tuple of User and Repo[]
...
const UserProfileComponent: FC<UserProfileProps> = ({ username }) => {
  const combinedUserData: [User, Repo[]] | null = useGithubUser(username);

  if (!combinedUserData) {
    return null;  // or a loading spinner
  }

  const userData: User = combinedUserData[0];
  const repoData:Repo[] = combinedUserData[1];
