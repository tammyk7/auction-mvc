...
- [Bootstrapping components](#bootstrapping-components)
- [Lifecycle events](#lifecycle-events)
- [Thread-safe vs. Non-thread-safe logging](#thread-safe-vs-non-thread-safe-logging)
...
(notice we also will have to define the EchoService, earlier in that same hydra file or better imported from a different hydra file:
```fsharp
type EchoRequest = {
    body: string
}
...
- Component lifecycle events:
    - `notifyAvailable()`: This method should be called when the current component becomes available.
    - `notifyUnavailable()`: This method should be called when the current component becomes unavailable.
    - `notifyClosed()`: This method should be called when the component is closed.
