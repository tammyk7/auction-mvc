...
- [How to define service contracts](#how-to-define-service-contracts)
- [Passthrough services](#passthrough-services)
- [Understanding generated code](#understanding-generated-service-code)
- [How to send/receive messages](#how-to-sendreceive-messages)
- [Backward compatible services](#backwards-compatible-services)
- [Hydra tooling and IDE plugins](#hydra-tooling-and-ide-plugin)
...
- **Interaction Models**: After defining the types that will be used for an endpoint, let's declare how we want the
  ...
  be defined through Hydra as service methods. In Hydra the call to a service is asynchronous (i.e. non-blocking), allowing the caller to perform other tasks immediately after the method call.
  Unless the service method is a fire and forget (which doesn't have a response), its response must be handled in a different method. In order to facilitate relating the method that handles the response to the request that did call the service method,
  ...
  implementation. To find out more about additional interaction models and how to define
